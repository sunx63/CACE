#set_init_noshare() gives the initial values when lambda:=1. It is used for r=3, no random effect sharing
#inputs:
#data: dataframe containing outcome, treatment assignment and receipt, cluster/site level variable 
#r: full dimension of random effects of Y model. default is 3.
...
set_init_noshare <- function(data,r=3,side=1,formula.c,formula.y,subname,method,col.clinic,col.trt,col.D,col.Y){
  ...
  return(list(alpha.intercept.init=alpha.init,tau.init=tau.init,C.init=C.init,delta.init=delta.init,estimatePC=P.c,
              y.itt.est=y.itt.est))
}#end of function

#Values
#alpha.intercept.init = initial values for an, ac0, ac1
#y.itt.est[2:length(init.list$y.itt.est)] = initial values for the coefficients for the covariates in Y model
